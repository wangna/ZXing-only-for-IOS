...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense {
  ...
  if (self) {
   ...
   // self.wantsFullScreenLayout = YES;
    ...
    CGRect rect = CGRectMake(0, 100, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 100);
...
//  pImageView.image = [UIImage initWithImage:@"文字框@2x.png"];
//  [self.view addSubview:overlayView];
//    
//  if(pTime == nil)
//  {
//      pTime = [NSTimer scheduledTimerWithTimeInterval:1.5 target:self selector:@selector(scrollTimer) userInfo:nil repeats:YES];
//  }
